...
/** @addtogroup 425_TMR_external_clock TMR_external_clock
  * @{
  */
...
int main(void)
{
  ...
  tmr1 pwm frequency = 96000000(systemclock)/96(period+1)/1000(div+1) = 1 khz
  tmr3 pwm frequency = 1000(tmr1 pwm frequency)/96(period+1)/1(div+1) = 10.42 hz */

  /* tmr1 and tmr3 counter mode configuration */
  tmr_base_init(TMR1, 95, 999);
  tmr_cnt_dir_set(TMR1, TMR_COUNT_UP);

  tmr_base_init(TMR3, 95, 0);
  ...
  tmr_channel_value_set(TMR1, TMR_SELECT_CHANNEL_1, 80);
  /* tmr3 channel 1 */
  tmr_output_channel_config(TMR3, TMR_SELECT_CHANNEL_1, &tmr_output_struct);
  tmr_channel_value_set(TMR3, TMR_SELECT_CHANNEL_1, 80);
